...
load('Trained_QPSK_mm_R64T4.mat');
...
%                     u(:,uu) = [up;max(utemp,0) - max(utemp-1,0)];
            ...
            for uu = 1:Nu
                ...
                X_temp((Ns*(uu-1)+1):Ns*uu,:) = real_X_temp + sqrt(-1)*imag_X_temp;
            end
    ...
    FER_100(snr) = sum(FER_100_cnt(:,snr))/Nfrm_vec(snr)/Nu;
    FER_200(snr) = sum(FER_200_cnt(:,snr))/Nfrm_vec(snr)/Nu;
    FER_300(snr) = sum(FER_300_cnt(:,snr))/Nfrm_vec(snr)/Nu;
    ADMM_ave(snr) = sum(ADMM_cnt(:,snr))/Nfrm_vec(snr)/Nu;
end