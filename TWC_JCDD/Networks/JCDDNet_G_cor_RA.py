...
class JCDD_G_cor_layer(layers.Layer):
    def __init__(self):
        super(JCDD_G_cor_layer, self).__init__()

    def build(self,input_shape):
        if qAry == 2:
            self.miu = self.add_weight(name='miu',shape=(1,1),initializer=keras.initializers.Constant(value=0.7),trainable=True) #constraint=constraints.non_neg(),
            self.alpha = self.add_weight(name='alpha',shape=(1,1),initializer=keras.initializers.Constant(value=7),trainable=True)
        elif qAry == 4:
            self.miu = self.add_weight(name='miu',shape=(1,1),initializer=keras.initializers.Constant(value=0.08),trainable=True) #constraint=constraints.non_neg(),
            self.alpha1 = self.add_weight(name='alpha1',shape=(1,1),initializer=keras.initializers.Constant(value=1.1),trainable=True)
            self.alpha2 = self.add_weight(name='alpha2',shape=(1,1),initializer=keras.initializers.Constant(value=1.1),trainable=True)
        self.factor = self.add_weight(name='factor',shape=(1,1),initializer=keras.initializers.Constant(value=1),trainable=True)
        self.cor = self.add_weight(name='cor',shape=(1,1),initializer=keras.initializers.Constant(value=1),trainable=True)
    ...
    def call(self, inputs):
        ...
        R = tf.matmul(XXp,XXp,transpose_b=True) + self.cor*sigma2_I
...
def JCDD_G_cor(Lnum) -> Model:
    ...
    for ll in range(Lnum):
        [lambda_W,F,u,z,lambda1,acc_new] = JCDD_G_cor_layer()([sigma2_I,Y,YYp,real_Xp,imag_Xp,lambda_W,F,u,z,lambda1,acc_new])

    u_est = u[:,:LDPC_N,:] - 0.5
    u_est = tf.nn.tanh(200*u_est)
    out = tf.concat([u_est,u,z],1)
    model = Model(inputs=inpt, outputs=out, name='JCDDNetGcor')
