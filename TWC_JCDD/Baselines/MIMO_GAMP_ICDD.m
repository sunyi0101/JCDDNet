...
%         MSE_INI = mean(mean(abs(Hest - H).^2))
        ...
        for nn = 1:Ld
            ...
            for uu = 1:Nu
                for kk = 1:Nt
                    DemodLLR((qAry*Nt*(nn-1)+qAry*(kk-1)+1):(qAry*Nt*(nn-1)+qAry*kk),uu) = LLR_temp(Nt*(uu-1)+kk,:);
                end
            end
        end
...
%                 MSE_ITR = mean(mean(abs(Hest-H).^2))
            ...
            for nn = 1:Ld
                InLLR_temp = zeros(Nt*Nu,qAry);
                for uu = 1:Nu
                    for kk = 1:Nt
                        InLLR_temp(Nt*(uu-1)+kk,:) = InLLR((qAry*Nt*(nn-1)+qAry*(kk-1)+1):(qAry*Nt*(nn-1)+qAry*kk),uu);
                    end
                end
                [xtemp,LLR_temp] = GAMP_SISO(sm(:,nn),sv(:,nn),Y(:,nn),Hest,sigma2,qAry,bit,signal,InLLR_temp);
                for uu = 1:Nu
                    for kk = 1:Nt
                        LLR((qAry*Nt*(nn-1)+qAry*(kk-1)+1):(qAry*Nt*(nn-1)+qAry*kk),uu) = LLR_temp(Nt*(uu-1)+kk,:);
                    end
                end
            end
            DemodLLR = LLR-InLLR;
